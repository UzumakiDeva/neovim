	...
	config = function()
		...
		vim.api.nvim_create_autocmd("LspAttach", {
			...
			callback = function(event)
				...
				map("<leader>sp", function()  require('telescope.builtin').find_files({
					search_dirs = { "/home/galaxy/" }
				}) end, "Search files from home")
		...
		local servers = {
			intelephense = {
				filetypes = { "php" }, -- Ensure it's enabled for PHP
				settings = {
					intelephense = {
						files = {
							maxSize = 1000000, -- Allow larger PHP files
						},
						format = {
			...
			handlers = {
				function(server_name)
					...
					if server_name == "intelephense" then
						server.root_dir =
							lspconfig.util.root_pattern("index.php", ".git", "composer.json", "package.json")
						server.settings.intelephense.format.enable = false
					end
